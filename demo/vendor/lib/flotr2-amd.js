(function (root, factory) {
    ...
    Flotr.defaultOptions = {
      ...
      xaxis: {
        ...
        customTick:{}          //自定义字段,用于制定需要变色的线条,key值为水平值,value为变色线条的颜色
      },
      x2axis: {},
      yaxis: {
        ticks: null,           // => format: either [1, 3] or [[1, 'a'], 3]
        ...
        titleAlign: 'center',
        margin: true,           // => Turn off margins with false
        customTick:{}          //自定义字段,用于制定需要变色的线条,key值为竖直值,value为变色线条的颜色
      },
      y2axis: {
        titleAngle: 270
      },
    ...
    //  try {
        ...
        if(options.fixFrame){
          E.fire(this.el, 'flotr:drawframe',[this]);
        }
    ...
    Graph.prototype = {
      ...
      calculateSpacing: function(){
        ...
        if(y.options.margin === false && options.fixFrame){
      ...
      _initCanvas: function(){
        ...
        for (i = el.parentElement.children.length; i--;) {
          child = el.parentElement.children[i];
          if (!this.canvas_div && child.className === 'flotr-frame') {
            this.canvas_div = child;
          }
        }
        ...
        if(!this.canvas_div){
          this.canvas_div = D.create("div");
          this.canvas_div.className = "flotr-frame";
          var parent = el.parentElement;
          D.insert(parent,this.canvas_div);
          D.insert(parent,el);
        }
        // Main canvas for drawing graph types
        this.canvas = getCanvas(this.canvas, 'canvas',this);
        // Overlay canvas for interactive features
        this.overlay = getCanvas(this.overlay, 'overlay',this);
        ...
        function getCanvas(canvas, name, context){
          if(!canvas){
        ...
        function drawGridLines (ticks, callback) {
          _.each(_.pluck(ticks, 'v'), function(v){
            ...
            if ((v <= a.min || v >= a.max) || 
              ...
              return;
            if(horizontalLines && options.yaxis.customTick[v]){
              ctx.stroke();
              var tempContext = me.canvas.getContext("2d");
              tempContext.beginPath();
              tempContext.strokeStyle = options.yaxis.customTick[v];
              var tempY = Math.floor(a.d2p(v)) + ctx.lineWidth/2;
              tempContext.moveTo(0, tempY);
              tempContext.lineTo(plotWidth, tempY);
              tempContext.stroke();
              ctx.stroke();
              ctx.beginPath();
              ctx.strokeStyle = grid.tickColor;
            }else if(verticalLines && options.xaxis.customTick[v]){
              ctx.stroke();
              var tempContext = me.canvas.getContext("2d");
              tempContext.beginPath();
              tempContext.strokeStyle = options.xaxis.customTick[v];
              var tempX = Math.floor(a.d2p(v)) + ctx.lineWidth/2;
              tempContext.moveTo(tempX, 0);
              tempContext.lineTo(tempX, plotHeight);
              tempContext.stroke();
              ctx.stroke();
              ctx.beginPath();
              ctx.strokeStyle = grid.tickColor;
    ...
    Flotr.addPlugin('drawframe', {
      options: {
        mode: null,
        hideCursor: true       
      },
      callbacks: {
        'flotr:drawframe': function() {
      ...
      drawFrame:function(){
        var p = this.plotOffset;
        // var plotWidth  = this.canvasWidth - p.left - p.right -20;
        var plotHeight = this.canvasHeight - p.bottom - p.top;
        var lw = this.options.grid.outlineWidth,
          orig = 0.5-lw+((lw+1)%2/2);
        //这里加入zepto依赖
        var marginTop = $(this.el).css("margin-top");
        $(this.canvas_div).css("margin-top",marginTop);
        $(this.canvas_div).css("margin-left",orig+p.left);
        D.setStyles(this.canvas_div,{position:"absolute",height: plotHeight+"px",width: this.el.parentElement.clientWidth-p.left-orig+"px",border:"1px solid black"});
      }
    });
    })();
